...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare MOSS-TTS finetuning JSONL by extracting target audio codes.")
    ...
    parser.add_argument("--encode-reference-audio", dest="encode_reference_audio", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument("--no-encode-reference-audio", dest="encode_reference_audio", action="store_false", help=argparse.SUPPRESS)
    parser.add_argument("--encode-ref-audio", dest="encode_reference_audio", action="store_true", help=argparse.SUPPRESS)
    parser.add_argument(
        "--save-shard-suffix",
        action="store_true",
...
def batch_encode(processor, paths: List[str], batch_size: int, n_vq: Optional[int], desc: str) -> List[torch.Tensor]:
...
def attach_reference_audio_codes(records: List[Dict[str, Any]], path_to_codes: Dict[str, List[List[int]]]) -> None:
    for record in records:
        ...
        if reference is not None:
            record["reference_audio_codes"] = [None if path is None else path_to_codes[path] for path in reference]
...
def main() -> None:
    ...
    if not records:
        raise ValueError(f"No records found for shard rank={rank} / world_size={world_size} in {args.input_jsonl}.")

    install_torchaudio_load_fallback()
    processor = AutoProcessor.from_pretrained(args.model_path, trust_remote_code=True, codec_path=args.codec_path)
    ...
    if args.encode_reference_audio:
        ...
        if unique_reference_paths:
            ...
            reference_code_map = {
                path: codes.tolist() for path, codes in zip(unique_reference_paths, reference_codes)
            }
