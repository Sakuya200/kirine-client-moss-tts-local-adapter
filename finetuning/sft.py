...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Supervised finetuning for MossTTSLocal-family tasks.")
...
def parse_channelwise_loss_weight(spec: Optional[str], n_heads: int) -> Optional[List[float]]:
    ...
    if len(values) == n_heads:
        ...
        raise ValueError(
            f"`channelwise_loss_weight` expects either {n_heads} values or 2 values (text_weight,total_audio_weight), got {len(values)}."
        )
    if sum(resolved) <= 0:
...
def build_processor(model_path: str, codec_path: str, audio_tokenizer_device: Optional[str], default_audio_tokenizer_device: str):
    install_torchaudio_load_fallback()
    processor = AutoProcessor.from_pretrained(model_path, trust_remote_code=True, codec_path=codec_path)
...
def resolve_model_asset(model_path: str, filename: str) -> Optional[Path]:
    model_path_obj = Path(model_path)
    if model_path_obj.is_dir():
        candidate = model_path_obj / filename
        return candidate if candidate.exists() else None
    try:
        resolved = cached_file(model_path, filename, _raise_exceptions_for_missing_entries=False)
    except OSError:
        return None
    if resolved is None:
        return None
...
def copy_support_files(model_path: str, output_dir: Path) -> None:
    for filename in SUPPORT_FILES:
        src = resolve_model_asset(model_path, filename)
...
def copy_inference_assets(model_path: str, codec_path: str, output_dir: Path) -> None:
    ...
    with open(output_dir / "processor_config.json", "w", encoding="utf-8") as file:
        json.dump(processor_config, file, indent=2, ensure_ascii=False)
...
def save_checkpoint(
    ...
    if accelerator.is_main_process:
        copy_support_files(model_path, output_dir)
        copy_inference_assets(model_path, codec_path, output_dir)
        with open(output_dir / "finetune_args.json", "w", encoding="utf-8") as file:
            json.dump(train_args, file, indent=2, ensure_ascii=False)
    accelerator.wait_for_everyone()
...
def shard_paths_for_rank(paths: List[Path], world_size: int, rank: int) -> tuple[List[Path], bool]:
    ...
    if not selected:
        raise ValueError(
            f"No shard assigned for rank={rank} world_size={world_size}. Please check --train-jsonl shard files and distributed config."
        )
    return selected, True


def load_jsonl_for_rank(spec: str, world_size: int, rank: int) -> tuple[List[Path], List[Dict[str, Any]], List[Path], bool]:
    all_paths = resolve_jsonl_paths(spec)
    rank_paths, using_pre_sharded_files = shard_paths_for_rank(all_paths, world_size=world_size, rank=rank)
...
def main() -> None:
    ...
    if need_audio_tokenizer:
        accelerator.print(
            f"[{format_timestamp()}] [sft] found records without precomputed reference audio codes; keeping audio_tokenizer for on-the-fly reference encoding."
        )
    ...
    if n_heads <= 1:
        raise ValueError("Failed to resolve model.config.n_vq for MOSS training.")
    resolved_channelwise_loss_weight = parse_channelwise_loss_weight(args.channelwise_loss_weight, n_heads)
    accelerator.print(
        f"[{format_timestamp()}] [sft] resolved channelwise_loss_weight={resolved_channelwise_loss_weight}"
    )
    ...
    if args.wandb_project and accelerator.is_main_process:
        ...
        init_kwargs: Dict[str, Any] = {"project": args.wandb_project, "config": train_args_to_save}
    ...
    try:
        _training_loop(
            ...
            resolved_channelwise_loss_weight=resolved_channelwise_loss_weight,
            max_train_steps=max_train_steps,
            global_batch_size=global_batch_size,
            output_root=output_root,
            train_args_to_save=train_args_to_save,
...
def _training_loop(
        ...
        for batch in train_dataloader:
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.logging_steps == 0:
                    ...
                    eta_seconds = max(max_train_steps - global_step, 0) / max(steps_per_sec, 1e-12)
                    ...
                    accelerator.print(
                        ...
                        f"loss={logged_loss:.4f} lr={lr_val:.2e} step_time={step_time:.2f}s "
                        f"steps_per_sec={steps_per_sec:.3f} samples_per_sec={samples_per_sec:.2f} "
    ...
    accelerator.print(
        f"[{format_timestamp()}] Finished training: global_step={global_step}, saved_epochs={completed_epochs}, output_dir={output_root}"
    )
